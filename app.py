...
with st.sidebar:
    ...
    st.divider()
    st.header("Kalite Filtresi Katmani")
    ENABLE_QUALITY_LAYER = st.checkbox("Kalite filtresini kullan", True)
    st.caption(
        "Bu katman yalnizca fiyat/hacim ve corporate action proksileri kullanir. "
        "Gercek haber, float, offering ve fundamentals verisi YOKTUR."
    )
    Q_MIN_HISTORY_DAYS = st.number_input("Min history (gun)", 60, 400, 120, 10)
    Q_MIN_MED_DVOL_20 = st.number_input(
        "Min median $ hacim (20g)", 100_000, 100_000_000, 2_000_000, 100_000
    )
    Q_MAX_ATR14_PCT = st.slider("Max ATR14 / Close (%)", 5.0, 60.0, 22.0, 1.0) / 100
    Q_MAX_CRASH_DAYS_60 = st.number_input("Max -20% gun sayisi (60g)", 0, 10, 2, 1)
    Q_MAX_GAPDOWN_DAYS_60 = st.number_input("Max -15% gap down sayisi (60g)", 0, 10, 2, 1)
    Q_REJECT_RECENT_CORP_ACTION = st.checkbox("Son corporate action / split suphelerini ele", True)
    Q_ACTION_LOOKBACK_DAYS = st.number_input("Corporate action lookback (gun)", 30, 240, 120, 10)
    Q_ACTION_RATIO_THRESH = st.slider("Corporate action ratio esigi", 1.01, 3.00, 1.20, 0.01)
...
def compute_features(df: pd.DataFrame, spy_df: pd.DataFrame, raw_df: Optional[pd.DataFrame] = None) -> Optional[dict]:
    """
    ...
    out.update(compute_quality_profile(df, raw_df))
    return out
...
def compute_quality_profile(df: pd.DataFrame, raw_df: Optional[pd.DataFrame] = None) -> dict:
    """
    Fiyat/hacim ve corporate action proksilerinden kalite profili uretir.
    Not: Bu katman HABER/FLOAT/OFFERING verisi kullanmaz; yalnizca teknik
    proksiler kullanir. Dolayisiyla "kalite" yorumu kismi ve ihtiyatlidir.
    """
    if df is None or df.empty:
        return {
            "history_bars": 0,
            "med_dollar_vol_20": np.nan,
            "atr14_pct": np.nan,
            "crash_days_60": 0,
            "gapdown_days_60": 0,
            "corp_action_ratio_range": np.nan,
            "recent_action_flag": False,
        }

    d = df.copy().sort_index()
    dollar_vol = d["close"] * d["volume"]
    med_dollar_vol_20 = dollar_vol.rolling(20).median().iloc[-1] if len(d) >= 20 else np.nan

    atr14_s = atr(d, 14)
    atr14_val = atr14_s.iloc[-1] if len(atr14_s) else np.nan
    last_close = float(d["close"].iloc[-1]) if len(d) else np.nan
    atr14_pct = float(atr14_val / last_close) if pd.notna(atr14_val) and last_close > 0 else np.nan

    daily_ret = d["close"].pct_change()
    prev_close = d["close"].shift(1)
    gap_pct = np.where(prev_close > 0, (d["open"] - prev_close) / prev_close, np.nan)
    gap_pct = pd.Series(gap_pct, index=d.index)

    crash_days_60 = int((daily_ret <= -0.20).tail(60).sum()) if len(d) >= 2 else 0
    gapdown_days_60 = int((gap_pct <= -0.15).tail(60).sum()) if len(d) >= 2 else 0

    corp_action_ratio_range = np.nan
    recent_action_flag = False
    if raw_df is not None and not raw_df.empty:
        raw_close = raw_df["close"].reindex(d.index).ffill()
        ratio = raw_close / d["close"].replace(0, np.nan)
        recent = ratio.tail(int(Q_ACTION_LOOKBACK_DAYS)).dropna()
        if len(recent) >= 5:
            mx = recent.max()
            mn = recent.min()
            if pd.notna(mx) and pd.notna(mn) and mn > 0:
                corp_action_ratio_range = float(mx / mn)
                recent_action_flag = bool(corp_action_ratio_range >= Q_ACTION_RATIO_THRESH)

    return {
        "history_bars": int(len(d)),
        "med_dollar_vol_20": float(med_dollar_vol_20) if pd.notna(med_dollar_vol_20) else np.nan,
        "atr14_pct": float(atr14_pct) if pd.notna(atr14_pct) else np.nan,
        "crash_days_60": int(crash_days_60),
        "gapdown_days_60": int(gapdown_days_60),
        "corp_action_ratio_range": float(corp_action_ratio_range) if pd.notna(corp_action_ratio_range) else np.nan,
        "recent_action_flag": bool(recent_action_flag),
    }


def passes_quality_filters(q: dict) -> tuple[bool, str]:
    if not ENABLE_QUALITY_LAYER:
        return True, "OK"

    if int(q.get("history_bars", 0)) < int(Q_MIN_HISTORY_DAYS):
        return False, f"History < {int(Q_MIN_HISTORY_DAYS)}"
    med_dvol = q.get("med_dollar_vol_20", np.nan)
    if pd.isna(med_dvol) or med_dvol < Q_MIN_MED_DVOL_20:
        return False, "Median $ hacim dusuk"
    atr14_pct = q.get("atr14_pct", np.nan)
    if pd.notna(atr14_pct) and atr14_pct > Q_MAX_ATR14_PCT:
        return False, "ATR% asiri yuksek"
    if int(q.get("crash_days_60", 0)) > int(Q_MAX_CRASH_DAYS_60):
        return False, "Asiri cok crash-day"
    if int(q.get("gapdown_days_60", 0)) > int(Q_MAX_GAPDOWN_DAYS_60):
        return False, "Asiri cok gap-down"
    if Q_REJECT_RECENT_CORP_ACTION and bool(q.get("recent_action_flag", False)):
        return False, "Son corporate action/split supheli"
    return True, "OK"


def passes_quality_row(row) -> tuple[bool, str]:
    if not ENABLE_QUALITY_LAYER:
        return True, "OK"

    try:
        if int(row.get("history_bars", 0)) < int(Q_MIN_HISTORY_DAYS):
            return False, f"History < {int(Q_MIN_HISTORY_DAYS)}"
        med_dvol = row.get("med_dollar_vol_20", np.nan)
        if pd.isna(med_dvol) or med_dvol < Q_MIN_MED_DVOL_20:
            return False, "Median $ hacim dusuk"
        atr14_pct = row.get("atr14_pct", np.nan)
        if pd.notna(atr14_pct) and atr14_pct > Q_MAX_ATR14_PCT:
            return False, "ATR% asiri yuksek"
        if int(row.get("crash_days_60", 0)) > int(Q_MAX_CRASH_DAYS_60):
            return False, "Asiri cok crash-day"
        if int(row.get("gapdown_days_60", 0)) > int(Q_MAX_GAPDOWN_DAYS_60):
            return False, "Asiri cok gap-down"
        if Q_REJECT_RECENT_CORP_ACTION and bool(row.get("recent_action_flag", False)):
            return False, "Son corporate action/split supheli"
        return True, "OK"
    except Exception:
        return False, "Kalite verisi eksik"

# ============================================================
# ALPACA VERI INDIRME
# ============================================================
@st.cache_data(ttl=600, show_spinner=False)
def fetch_daily_bars(_client, symbol: str, days: int = 260, adjustment: str = "all") -> pd.DataFrame:
    try:
...
@st.cache_data(ttl=600, show_spinner=False)
def fetch_daily_bars_batch(_client, symbols: list[str], days: int = 260, adjustment: str = "all") -> dict[str, pd.DataFrame]:
    out = {}
    if not symbols:
        return out
    try:
...
def precompute_feature_series(df: pd.DataFrame, spy_df: pd.DataFrame, raw_df: Optional[pd.DataFrame] = None) -> pd.DataFrame:
    """
    ...
    # Kalite katmani proksileri
    out["dollar_vol"] = out["close"] * out["volume"]
    out["med_dollar_vol_20"] = out["dollar_vol"].rolling(20).median()
    out["atr14_pct"] = np.where(out["close"] > 0, out["atr14"] / out["close"], np.nan)
    out["daily_ret"] = out["close"].pct_change()
    out["crash_days_60"] = (out["daily_ret"] <= -0.20).astype(int).rolling(60).sum()
    out["gapdown_days_60"] = (out["gap_pct"] <= -0.15).astype(int).rolling(60).sum()
    out["history_bars"] = np.arange(1, len(out) + 1)

    if raw_df is not None and not raw_df.empty:
        raw_close = raw_df["close"].reindex(out.index).ffill()
        out["corp_action_ratio"] = np.where(out["close"] > 0, raw_close / out["close"], np.nan)
        ratio_max = out["corp_action_ratio"].rolling(int(Q_ACTION_LOOKBACK_DAYS), min_periods=5).max()
        ratio_min = out["corp_action_ratio"].rolling(int(Q_ACTION_LOOKBACK_DAYS), min_periods=5).min()
        out["corp_action_ratio_range"] = np.where(ratio_min > 0, ratio_max / ratio_min, np.nan)
        out["recent_action_flag"] = (out["corp_action_ratio_range"] >= Q_ACTION_RATIO_THRESH).fillna(False)
    else:
        out["corp_action_ratio"] = np.nan
        out["corp_action_ratio_range"] = np.nan
        out["recent_action_flag"] = False
    ...
    feat_cols = [
        ...
        "dollar_vol", "med_dollar_vol_20", "atr14_pct",
        "crash_days_60", "gapdown_days_60", "history_bars",
        "corp_action_ratio", "corp_action_ratio_range", "recent_action_flag",
    ]
    ...
    if st.button("Taramayi Baslat", type="primary"):
        ...
        if not universe:
            ...
            for i in range(0, total, batch_size):
                ...
                bars_map = fetch_daily_bars_batch(client, chunk, days=260, adjustment="all")
                all_bars.update(bars_map)
                if ENABLE_QUALITY_LAYER and Q_REJECT_RECENT_CORP_ACTION:
                    raw_chunk = fetch_daily_bars_batch(client, chunk, days=260, adjustment="raw")
                    raw_bars.update(raw_chunk)
            ...
            for sym, df in all_bars.items():
                if sym == "SPY":
                    continue
                raw_df = raw_bars.get(sym, pd.DataFrame()) if raw_bars else None
                feats = compute_features(df, spy_df, raw_df=raw_df)
                if feats is None:
                ...
                q_passed, q_reason = passes_quality_filters(feats)
                if not q_passed:
                    rejected.append({"symbol": sym, "reason": q_reason, "score": score})
                    continue
                ...
                candidates.append({
                    ...
                    "Q_MedDV20_$": round(feats["med_dollar_vol_20"], 0) if pd.notna(feats["med_dollar_vol_20"]) else None,
                    "Q_ATR14_%": round(feats["atr14_pct"] * 100, 2) if pd.notna(feats["atr14_pct"]) else None,
                    "Q_Crash60": int(feats["crash_days_60"]),
                    "Q_GapDn60": int(feats["gapdown_days_60"]),
                    "Q_Action?": bool(feats["recent_action_flag"]),
...
with tab2:
    ...
    if st.button("Backtesti Baslat", type="primary"):
        try:
            ...
            raw_bars_map: dict[str, pd.DataFrame] = {}
            step = 50  # Daha kucuk batch, hata izolasyonu icin
            progress = st.progress(0.0, text="0/0")
            total = len(symbols)
            for i in range(0, total, step):
                chunk = symbols[i:i + step]
                got = fetch_daily_bars_batch(client, chunk, days=bt_days + 260, adjustment="all")
                bars_map.update(got)
                if ENABLE_QUALITY_LAYER and Q_REJECT_RECENT_CORP_ACTION:
                    raw_chunk = fetch_daily_bars_batch(client, chunk, days=bt_days + 260, adjustment="raw")
                    raw_bars_map.update(raw_chunk)
            ...
            for i, sym in enumerate(syms_list):
                try:
                    fs = precompute_feature_series(
                        bars_map[sym],
                        spy_df,
                        raw_df=raw_bars_map.get(sym, pd.DataFrame()) if raw_bars_map else None,
                    )
                    if not fs.empty:
                        feat_map[sym] = fs
                except Exception:
            ...
            for di, dt in enumerate(test_dates):
                for sym, fs in feat_map.items():
                    ...
                    if not passes_filters_row(row):
                        continue
                    q_passed, _ = passes_quality_row(row)
                    if not q_passed:
                        continue
                    score = score_row(row)
                    if score < MIN_SCORE:
...
with tab4:
    ...
    st.caption(
        ...
        "sadece fiyat-hacim anomalisi, volatilite sikismasi, birikim desenleri "
        "ve kalite katmani proksileri kullanilir."
    )
    ...
    if st.button("Patlayici Aday Taramasi", type="primary", key="btn_par_scan"):
        try:
            ...
            raw_par_bars: dict[str, pd.DataFrame] = {}
            prog = st.progress(0.0, text="0/0")
            step = 50
            total = len(par_universe)
            for i in range(0, total, step):
                chunk = par_universe[i:i + step]
                got = fetch_daily_bars_batch(client, chunk, days=300, adjustment="all")
                par_bars.update(got)
                if ENABLE_QUALITY_LAYER and Q_REJECT_RECENT_CORP_ACTION:
                    raw_chunk = fetch_daily_bars_batch(client, chunk, days=300, adjustment="raw")
                    raw_par_bars.update(raw_chunk)
                done = min(i + step, total)
            ...
            for idx, sym in enumerate(syms_list):
                try:
                    fs = precompute_feature_series(
                        par_bars[sym],
                        spy_df,
                        raw_df=raw_par_bars.get(sym, pd.DataFrame()) if raw_par_bars else None,
                    )
                    if fs.empty:
                        continue
                    ...
                    q_passed, q_reason = passes_quality_row(row)
                    if not q_passed:
                        par_rejected.append({"symbol": sym, "reason": q_reason})
                        continue
                    ...
                    par_candidates.append({
                        ...
                        "Q_MedDV20_$": round(float(row["med_dollar_vol_20"]), 0) if pd.notna(row["med_dollar_vol_20"]) else None,
                        "Q_ATR14_%": round(float(row["atr14_pct"]) * 100, 2) if pd.notna(row["atr14_pct"]) else None,
                        "Q_Crash60": int(row["crash_days_60"]) if pd.notna(row["crash_days_60"]) else None,
                        "Q_GapDn60": int(row["gapdown_days_60"]) if pd.notna(row["gapdown_days_60"]) else None,
                        "Q_Action?": bool(row["recent_action_flag"]),
                        "Entry": levels["entry"],
                        "Stop": levels["stop"],
                        "TP1": levels["tp1"],
                        "TP2": levels["tp2"],
    ...
    if st.button("Parabolik Backtest Baslat", type="primary", key="btn_par_bt"):
        try:
            ...
            for i in range(0, total, step):
                chunk = symbols[i:i + step]
                got = fetch_daily_bars_batch(client, chunk, days=pb_days + 300, adjustment="all")
                bars_map.update(got)
                if ENABLE_QUALITY_LAYER and Q_REJECT_RECENT_CORP_ACTION:
                    raw_chunk = fetch_daily_bars_batch(client, chunk, days=pb_days + 300, adjustment="raw")
                    raw_bars_map.update(raw_chunk)
                done = min(i + step, total)
                prog.progress(done / total, text=f"{done}/{total}")
            prog.empty()
            ...
            for di, dt in enumerate(test_dates):
                for sym, pf in feat_map.items():
                    ...
                    par_trades.append({
                        ...
                        "q_med_dvol20": round(float(row["med_dollar_vol_20"]), 0) if pd.notna(row["med_dollar_vol_20"]) else None,
                        "q_atr14_pct": round(float(row["atr14_pct"]) * 100, 2) if pd.notna(row["atr14_pct"]) else None,
                        "q_crash60": int(row["crash_days_60"]) if pd.notna(row["crash_days_60"]) else None,
                        "q_gapdn60": int(row["gapdown_days_60"]) if pd.notna(row["gapdown_days_60"]) else None,
                        "q_action": bool(row["recent_action_flag"]),
                    })
...
st.caption(
    ...
    "verir, garanti vermez. Kalite katmani float/news/fundamentals kullanmaz; "
    "yalnizca fiyat-hacim ve corporate action proksileri kullanir. Once paper "
    "account'ta test et, ardindan kucuk sermayeli canli teste gec. "
    "Parametreleri degistirmeden once backtest'te etkilerini mutlaka gor."
)
